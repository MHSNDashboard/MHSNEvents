...
class Event:
  ...
  def store_in_database(self):
    ...
    db["events"][self.id] = json_serializable_data
  ...
  def find_all(lmbda):
    ...
    for key in db["events"].keys():
      if lmbda(db["events"][key]): #lambda function should take values from each key in DB
        found.append(key)
...
class Advertisement:
  #Attributes of ads, due to change when HTML/CSS comes up for the website
  def __init__(self, id, header="", subheader="", subtext_right="", subtext_left="", image=None, image_data=None):
    self.id = id
    self.header = header
    self.subheader = subheader
  ...
  def store_in_database(self):
    json_serializable_data = {"id": self.id, "header" : self.header, "subheader" : self.subheader, 
                              "subright" : self.subtext_right, "subleft" : self.subtext_left, "image" : self.image, "image_data" : self.image_data}
    db["ads"][self.id] = json_serializable_data
  ...
  def find_all(lmbda):
    ...
    for key in db["ads"].keys():
      if lmbda(db["ads"][key]): #lambda function should take values from each key in DB
        found.append(key)
